    ...
    $.ajax(settings).done(function (data) {
        ...
        for (var i = 0; i < data.events.length; i++) {
            ...
            <h2>${data.events[i].name.text}</h2>
                ...
                var newDiv4 = $("<div>").addClass("tab-pane fade show description").attr({ id: "v-pills-" + i, role: "tabpannel", "aria-labelledby": "v-pills-" + i + "-tab" }).html(`
                <div class="row">
                <div class="col-md-9">
                <h2>${data.events[i].name.text}</h2>
            ...
            // ELSE conditional for no reslut
                ...
                $("#eventbrite").empty();
                $("#eventbrite").html("<h1 class='text-center text-white'>No results found</h2>");
            }
        }; // END loop for data
